...
export default function CollectionHero({
    ...
    return (
        <div className="relative w-full aspect-[16/10] sm:aspect-[21/9] max-h-[480px] overflow-hidden bg-gray-900">
            <Image
                src={imageUrl}
                alt={alt || title}
                fill
                ...
                <div className="flex-1 flex items-end md:items-center">
                    <div className={`${SITE_CONTAINER} w-full pb-8 md:pb-0 md:py-10`}>
                        <div className="max-w-2xl">
                            ...
                            <h1
                                className="text-3xl sm:text-4xl md:text-5xl lg:text-[3.25rem] font-bold text-white tracking-tight leading-[1.05] drop-shadow-lg"
                                style={{ fontFamily: "var(--font-playfair)" }}
                            >
                                {title}
                            </h1>
                            {description && (
                                <p className="text-sm md:text-base text-white/75 mt-3 max-w-lg leading-relaxed line-clamp-2 md:line-clamp-3">
                                    {description}
                                </p>
                            )}
                            {pieceLabel && (
                                <span className="inline-block mt-4 md:mt-5 px-3.5 py-1.5 rounded-full bg-white/10 backdrop-blur-sm border border-white/20 text-[11px] font-semibold text-white/90 tracking-wide">
                                    {pieceLabel}
                                </span>
                            )}
                        </div>
