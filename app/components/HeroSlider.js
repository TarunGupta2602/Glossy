...
export default function HeroSlider() {
    ...
    const visibleIndexes = useMemo(() => {
        // Only decode active + neighbors to cut LCP bandwidth
        ...
        >
                    ...
                    return (
                        ...
                        >
                            <Image
                                ...
                                className={`object-cover object-[center_20%] sm:object-[center_22%] md:object-[center_30%] transition-transform duration-[8s] ease-out ${
            ...
            >
                <div className="max-w-2xl">
                    ...
                    <h1
                        key={`h-${currentIdx}`}
                        className="hero-copy-in text-[1.55rem] sm:text-4xl md:text-5xl lg:text-[3.35rem] font-playfair font-bold text-white tracking-tight leading-[1.1] mb-2.5 sm:mb-5"
                    >
                    ...
                    <p
                        key={`p-${currentIdx}`}
                        className="hero-copy-in text-[13px] sm:text-base md:text-lg text-white/85 max-w-lg leading-relaxed mb-5 sm:mb-9 line-clamp-2 sm:line-clamp-none"
                    >
                        {activeSlide.support}
                    </p>
                    ...
                    <div
                        key={`c-${currentIdx}`}
                        className="hero-copy-in flex flex-row flex-wrap items-center gap-2.5 sm:gap-4"
                    >
                        <Link
                            href={activeSlide.primary.href}
                            className="inline-flex items-center justify-center gap-2 bg-[#E91E63] text-white text-[10px] sm:text-xs font-black uppercase tracking-[0.14em] sm:tracking-[0.18em] px-4 sm:px-6 py-3 sm:py-4 hover:bg-[#c2185b] active:scale-[0.98] transition-all duration-200 min-h-11"
                        >
                        ...
                        <Link
                            ...
                            className="inline-flex items-center justify-center text-white text-[10px] sm:text-xs font-black uppercase tracking-[0.14em] sm:tracking-[0.18em] px-2 py-3 border-b border-white/40 hover:border-[#E91E63] transition-colors duration-200 min-h-11"
                        >
            ...
            <div className="absolute bottom-3 right-3 sm:bottom-10 sm:right-10 z-20 flex items-center gap-3">
                <div className="flex sm:hidden items-center gap-2">
                    <button
                        type="button"
                        onClick={prev}
                        className="w-9 h-9 rounded-full border border-white/30 bg-black/30 backdrop-blur-sm text-white flex items-center justify-center active:scale-95"
                ...
                <div className="hidden sm:flex items-center gap-2">
                    <button
                        type="button"
                        onClick={prev}
                        className="w-10 h-10 rounded-full border border-white/30 bg-black/25 backdrop-blur-sm text-white flex items-center justify-center hover:bg-black/40 transition-colors"
                        aria-label="Previous slide"
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
