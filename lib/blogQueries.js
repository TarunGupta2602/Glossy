 ...
 *  Admin/CMS rows win over in-repo static posts on the same slug.
 */
export async function findBlogBySlug(supabase, slug, select = "*") {
    ...
    if (supabase) {
        ...
        if (exact) {
            return {
                blog: exact,
                requested,
                canonicalSlug: normalizeBlogSlug(exact.slug) || exact.slug,
            };
        }
    ...
    const staticBlog = getStaticBlogBySlug(requested) || getStaticBlogBySlug(normalized);
    if (staticBlog) {
        return {
            blog: staticBlog,
            requested,
            canonicalSlug: staticBlog.slug,
        };
    }

    return { blog: null, requested, canonicalSlug: normalized || requested };
}

/** Merge CMS + static posts. Same slug: CMS wins. Sorted newest first. */
export function mergeBlogFeeds(dbBlogs = [], staticSummaries = []) {
    ...
    for (const post of staticSummaries || []) {
        if (post?.slug && !cmsSlugs.has(post.slug)) {
            merged.push(post);
        }
    }
...
export async function getRelatedBlogPosts(supabase, blog, limit = 3) {
    ...
    const { STATIC_BLOG_POSTS } = await import("@/lib/staticBlogPosts");
    const staticCandidates = STATIC_BLOG_POSTS.map((post) => ({
        id: post.id,
        title: post.title,
        slug: post.slug,
        image: post.image,
    ...
    const list = mergeBlogFeeds(candidates, staticCandidates).filter(
        (post) => post.id !== blog.id && post.slug !== blog.slug
    );
