...
/**
 * Google Business Profile URL — set NEXT_PUBLIC_GOOGLE_BUSINESS_URL in env once GBP is live.
 * Leave empty until the profile exists (avoids linking to a broken URL).
 */
export const GOOGLE_BUSINESS_URL = process.env.NEXT_PUBLIC_GOOGLE_BUSINESS_URL || "";

/** Trust signals when a product has zero reviews (config — not invented per-SKU). */
