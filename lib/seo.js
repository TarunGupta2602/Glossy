...
export function buildProductSeo({ name, description, categoryName, price, imageAlt }) {
    const category = categoryName || "Jewelry";
    const priceNum = Number(price) || 0;

    const meta_title = `${name} | ${category}`;
    const meta_description = description?.trim()
        ? description.trim().slice(0, 160)
        : `Shop ${name} from our ${category} collection at The Luxe Jewels. Premium anti-tarnish, waterproof jewelry at ₹${priceNum.toLocaleString("en-IN")}. Buy 2 Get 1 Free + free shipping across India.`;
